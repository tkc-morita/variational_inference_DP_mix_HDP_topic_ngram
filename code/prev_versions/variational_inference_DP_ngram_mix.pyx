...
def make_variational_inference( # Wrapper
			sl_T,
			customers,
			n,
			T_base,
...
cdef int num_symbols
cdef np.float64_t[:,:] error_mat
...
cdef class VariationalInference(object):
	cdef list customers
	cdef np.float64_t[:,:] varpar_assignment
	cdef np.float64_t[:] concent_priors
	cdef np.float64_t[:] varpar_concent
	cdef np.float64_t concent_mean
	cdef np.float64_t[:,:] varpar_stick
	cdef np.float64_t[:] sum_gamma
	cdef np.float64_t[:,:] digamma_diffs
	cdef int T
	cdef list hdp_ngrams
	cdef bytes result_path
	cdef np.float64_t current_var_bound
	def __init__(
			self,
			int T,
			list customers,
# 			concent,
			int n,
			int T_base,
			np.ndarray[np.float64_t, ndim=1] concent_priors,
# 			noise,
			bytes result_path
			):
...
# 		assert noise_per_symbol, 'noise/(num_symbols-1) is to small to represent.'
		em = np.full((num_symbols,)*2, noise_per_symbol)
		np.fill_diagonal(
						em,
						1-noise
						)
...
# 		logger.info("Logger set up.")
	...
	def train(self, int max_iters, np.float64_t min_increase):
		...
		cdef np.float64_t new_var_bound, improvement
...
# 		df_assignment_sl.to_csv(self.result_path+"SubLexica_assignment.csv", index=False, encoding='utf-8')
		np.savetxt(
			self.result_path+"SubLexica_assignment.csv",
			np.append(
				self.varpar_assignment,
				np.append(
					np.max(self.varpar_assignment, axis=1)[:,np.newaxis],
					np.arange(self.varpar_assignment.shape[0])[:,np.newaxis],
					axis=1
					),
				axis=1
			),
			delimiter=",",
			header=','.join(map(str, xrange(self.varpar_assignment.shape[1])))+',most_probable_sublexicon,customer_id'
			)
...
# 		df_stick_sl.to_csv(self.result_path+"SubLexica_stick.csv", index=False, encoding='utf-8')
		np.savetxt(
			self.result_path+"SubLexica_stick.csv",
			np.append(
				self.varpar_stick,
				np.arange(self.varpar_stick.shape[0])[:,np.newaxis],
				axis=1
			),
			delimiter=",",
			header='beta_par1,beta_par2,cluster_id'
			)
...
# 		df_concent.to_csv(self.result_path+"Sublexica_concent.csv", index=False, encoding='utf-8')
		np.savetxt(
			self.result_path+"Sublexica_concent.csv",
			self.varpar_concent,
			delimiter=",",
			header='shape,inverse_scale'
			)
...
#  										)
				np.savetxt(
					self.result_path+("sublex-%i_ngram-context_%s_assignment.csv" % (sblx_id,decoded_context)),
					np.append(
						bottom_rst.varpar_assignment,
						np.append(
							np.arange(bottom_rst.varpar_assignment.shape[0])[:,np.newaxis],
							np.array(bottom_rst.word_ids)[:,np.newaxis],
							axis=1
							),
						axis=1
					),
					delimiter=",",
					header=','.join(map(str, xrange(self.varpar_assignment.shape[1])))+',customer_id'
					)
...
#  								)
				np.savetxt(
					self.result_path+("sublex-%i_ngram-context_%s_stick.csv" % (sblx_id,decoded_context)),
					np.append(
						bottom_rst.varpar_stick,
						np.arange(bottom_rst.varpar_stick.shape[0])[:,np.newaxis],
						axis=1
					),
					delimiter=",",
					header='beta_par1,beta_par2,cluster_id'
...
#  								)
				np.savetxt(
					self.result_path+("sublex-%i_ngram-context_%s_label.csv" % (sblx_id,decoded_context)),
					np.append(
						bottom_rst.varpar_label,
						np.arange(bottom_rst.varpar_label.shape[0])[:,np.newaxis],
						axis=1
					),
					delimiter=",",
					header=','.join(map(str, xrange(self.varpar_assignment.shape[1])))+',cluster_id'
					)
...
#  								)
	...
	cdef _update_varpar_concent(self):
...
# 		self.concent_log_mean=sps.digamma(self.varpar_concent[0])-np.log(self.varpar_concent[1])
	...
	cdef _update_varpar_stick(self):
# 		self.varpar_stick[...,0] = np.add(np.sum(self.varpar_assignment, axis=0)[:-1],1)
# 		self.varpar_stick[...,1] = np.add(
# 										np.sum(
# 										np.cumsum(
...
# 										self.concent_mean
# 										)
		self.varpar_stick = np.array((
								np.add(np.sum(self.varpar_assignment, axis=0)[:-1],1),
								np.add(
										np.sum(
										np.cumsum(
											self.varpar_assignment[:,::-1],
...
# 		
# 		
	cdef _update_varpar_assignment(self):
		cdef np.ndarray[np.float64_t, ndim=2] log_varpar_assignment = (
									np.append(
...
# 		return self.hdp_ngrams.get_var_bound()
		cdef np.ndarray[np.float64_t, ndim=2] cumsum_phis = np.cumsum(
							self.varpar_assignment[...,::-1],
							axis=-1
							)[...,::-1]
		cdef HDPNgram table
...
# 						-
# 						table._get_E_log_q_varpar() # E[log q(eta)]
						table.get_var_bound()
...
# 						) # E[log q(alpha)]
				+
				(np.sum(np.multiply(self.digamma_diffs[:,0],np.subtract(self.varpar_stick[:,0],1)))
					+
					np.sum(np.multiply(self.digamma_diffs[:,1],np.subtract(self.varpar_stick[:,1],1)))
					-
					np.sum(
...
cdef class HDPNgram(object):
	cdef int id
	cdef VariationalInference wrapper
	cdef int n
	cdef dict tree
	cdef np.float64_t[:] concent_priors
	cdef readonly np.float64_t[:] varpar_concent_inv_scale
	cdef np.float64_t[:] varpar_concent_shape
	cdef np.float64_t[:] concent_mean
	def __init__(self,
					int T,
					int n,
	...
	def get_word_assignment_weights(self, np.int_t[:] word_ids):
		return np.array(self.wrapper.varpar_assignment)[word_ids,self.id]
# 		
# 		
# 	def _get_E_log_q_varpar(self):
# 		cdef dict level
# 		cdef object restaurant
# 		return np.sum(
# 					[restaurant._get_E_log_q_varpar()
# 						for level in self.tree.itervalues()
...
# 		return (
# 				np.sum(
# 					[restaurant._get_sum_E_log_p_varpars()
# 						for level in self.tree.itervalues()
# 							for restaurant in level.itervalues()
# 							]
# 					)
...
# 		return log_pred_prob
	...
	cdef set_varpar_assignment(self):
		cdef dict level
		cdef object restaurant
...
cdef class DP_top(object):
	cdef HDPNgram hdp
	cdef int context_length
	cdef readonly np.float64_t[:,:] varpar_stick
	cdef np.float64_t[:] sum_gamma
	cdef readonly np.float64_t[:,:] digamma_diffs
	cdef readonly np.float64_t[:,:] varpar_label
	cdef readonly np.float64_t[:,:] tau_x_error
	cdef readonly np.float64_t[:,:,:] phi_x_tau_x_error_3d
	cdef readonly np.float64_t[:,:,:] varpar_assignment_3d
	cdef int T
	cdef int new_id
	cdef list children
	cdef int num_children
	cdef int num_tables_child
	
	def __init__(self, int T, int context_length, HDPNgram hdp):
		self.hdp=hdp
...
# 		self.concent_log_mean=sps.digamma(self.varpar_concent[0])-np.log(self.varpar_concent[1])
	...
	def add_customer(self, DP child):
		self.children.append(child)
		cdef int issued_id = self.new_id
		self.new_id+=1
		return issued_id
	...
	def set_varpar_assignment(self):
		...
		self.varpar_assignment_3d = np.random.dirichlet(
											np.ones(self.T),
											(self.num_children, self.num_tables_child)
											) # phi in Blei and Jordan.
		self.phi_x_tau_x_error_3d=np.matmul(np.reshape(self.varpar_assignment_3d, (self.num_children*self.num_tables_child, self.T)),
										self.tau_x_error
										).reshape(
														self.num_children,
...
# 		self.concent_log_mean=sps.digamma(self.varpar_concent[0])-np.log(self.varpar_concent[1])
	...
	cdef _update_varpar_stick(self):
# 		self.varpar_stick[...,0] = np.add(np.sum(self.varpar_assignment_3d, axis=(0, 1))[:-1],1)
# 		self.varpar_stick[...,1] = np.add(
# 									np.sum(
# 										np.cumsum(
# 											self.varpar_assignment_3d[...,::-1],
# 											axis=-1
# 												)[...,::-1],
# 										axis=(0,1)
# 										)[1:],
...
# 										)
		self.varpar_stick = np.array((
							np.add(np.sum(self.varpar_assignment_3d, axis=(0, 1))[:-1],1),
							np.add(
								np.sum(
									np.cumsum(
										self.varpar_assignment_3d[...,::-1],
	...
	cdef _update_varpar_assignment(self, np.ndarray[np.float64_t, ndim=2] children_label_np):
		cdef np.ndarray[np.float64_t, ndim=2] log_varpar_assignment = (
									np.append(
# 										sps.digamma(self.varpar_stick[:,0])-digamma_gamma1plus2,
		...
		cdef np.ndarray[np.float64_t, ndim=2] varpar_assignment_2dim=np.exp(log_varpar_assignment-spm.logsumexp(log_varpar_assignment, axis=-1)[:,np.newaxis])
		self.varpar_assignment_3d=varpar_assignment_2dim.reshape(self.num_children, self.num_tables_child, self.T)
		self.phi_x_tau_x_error_3d=np.matmul(
	...
	cdef _update_varpar_label(self, np.ndarray[np.float64_t, ndim=2] children_label_np):
		cdef np.ndarray[np.float64_t, ndim=2] log_varpar_label = (
								np.matmul(
									np.matmul(
										np.transpose(np.reshape(self.varpar_assignment_3d, (
																	self.num_children*self.num_tables_child,
																	self.T
																	))
	...
	def update_varpars(self):
		cdef DP child
		cdef np.ndarray[np.float64_t, ndim=2] children_label_np = np.array([child.varpar_label for child in self.children]).reshape(
																self.num_children*self.num_tables_child,
																num_symbols
...
# 		self._update_varpar_concent()
	...
	def get_var_bound(self):
		return self._get_sum_E_log_p_varpars()-self._get_E_log_q_varpar()
		
	cdef _get_sum_E_log_p_varpars(self):
		cdef np.ndarray[np.float64_t, ndim=3] cumsum_phis = np.cumsum(
							self.varpar_assignment_3d[...,::-1],
							axis=-1
							)[...,::-1]
		return (
...
# 						+
					...
					np.sum(
							...
							+
							self.digamma_diffs[:,0]*np.sum(self.varpar_assignment_3d[...,:-1], axis=(0,1))
							) # E[log p(Z | V)]
				)

	cdef _get_E_log_q_varpar(self):
		return(
# 					(
# 						(self.concent_priors[0]-1)*self.concent_log_mean
# 						-
...
# 					+
					...
					+
					np.sum(self.varpar_assignment_3d*np.ma.log(self.varpar_assignment_3d)) # E[log q(eta)]
					)

cdef class DP(DP_top):
	cdef DP_top mother
	cdef int id
	
	def __init__(self, int T, DP_top mother, int context_length, HDPNgram hdp):
		self.mother = mother
# 		self.hdp=hdp
# 		self.context_length=context_length
# # 		self.concent_priors = concent_priors
...
# 		self.new_id = 0
		...
		super(DP,self).__init__(T, context_length, hdp)
...
# 											(self.num_children, self.num_tables_child)
# 											) # phi in Blei and Jordan.
# 		assert self.varpar_assignment_3d.size, 'Empty restaurant created.'
# 		self.phi_x_tau_x_error_3d=np.matmul(self.varpar_assignment_3d.reshape(self.num_children*self.num_tables_child, self.T),
# 										self.tau_x_error
# 										).reshape(
# 														self.num_children,
# 														self.num_tables_child,
...
# 		self.varpar_stick[...,0] = np.sum(self.varpar_assignment_3d, axis=(0, 1))[:-1]+1
...
# 	cdef _update_varpar_assignment(self, np.ndarray[np.float64_t, ndim=2] children_label_np):
# 		cdef np.float64_t[:,:] log_varpar_assignment = (
# 									np.append(
# 										self.digamma_diffs[:,0],
# 										0
# 										)[np.newaxis,:]
...
# 										)
# 								)
# 		cdef np.float64_t[:,:] varpar_assignment_2dim=np.exp(log_varpar_assignment-spm.logsumexp(log_varpar_assignment, axis=-1)[:,np.newaxis])
# 		self.varpar_assignment_3d=varpar_assignment_2dim.reshape(self.num_children, self.num_tables_child, self.T)
# 		self.phi_x_tau_x_error_3d=np.matmul(
# 									varpar_assignment_2dim,
# 									self.tau_x_error).reshape(
...
# 														)
	...
	cdef _update_varpar_label(self, np.ndarray[np.float64_t, ndim=2] children_label_np):
		cdef np.ndarray[np.float64_t, ndim=2] log_varpar_label = (
								self.mother.phi_x_tau_x_error_3d[self.id]
								+np.matmul(
									np.matmul(
										np.transpose(np.reshape(self.varpar_assignment_3d,(
																	self.num_children*self.num_tables_child,
...
# 	def update_varpars(self):
# 		cdef np.float64_t[:,:] children_label_np = np.array([child.varpar_label for child in self.children]).reshape(
# 																self.num_children*self.num_tables_child,
# 																num_symbols
# 																)# (num_children*num_tables_child)x|∑|
# # 		self._update_varpar_concent()
...
# 					np.sum(self.varpar_label*np.ma.log(self.varpar_label)) # E[log q(Z)]
# 					+
# 					np.sum(self.varpar_assignment_3d*np.ma.log(self.varpar_assignment)) # E[log q(eta)]
# 					)
		
cdef class DP_bottom(DP):
	cdef readonly np.float64_t[:,:] phi_x_tau_x_error
	cdef readonly np.float64_t[:,:] varpar_assignment
	cdef list customers_list
# 	cdef np.int_t[:] customers
	cdef list word_ids_list
	cdef readonly np.int_t[:] word_ids
	def __init__(self, int T, DP_top mother, int context_length, HDPNgram hdp):
# 		self.hdp=hdp
# 		self.mother = mother
# 		self.context_length=context_length
...
# 		self.new_id = 0
		...
		self.word_ids_list=[]
# 		self.id = self.mother.add_customer(self)
		super(DP_bottom,self).__init__(T, mother, context_length, hdp)
		
		
	def add_customer(self, target_value, word_id):
		issued_id = self.new_id
		self.new_id+=1
		self.customers_list.append(target_value)
		self.word_ids_list.append(word_id)
		return issued_id
		
		
	def set_varpar_assignment(self):
		self.word_ids=np.array(self.word_ids_list)
		self.word_ids_list=[]
# 		self.customers=np.array(self.customers_list)
# 		self.customers_list=[]
		self.error_mat_obs = np.array(error_mat)[self.customers_list]
		self.varpar_assignment = np.random.dirichlet(np.ones(self.T), len(self.customers_list)) # phi in Blei and Jordan.
...
# 										)
	...
	cdef _update_varpar_assignment_btm(self):
		cdef np.ndarray[np.float64_t, ndim=2] log_varpar_assignment = (
									np.append(
										self.digamma_diffs[:,0],
										0
									...
									+(np.matmul(
# 										np.array(error_mat)[self.customers], # Nx|∑|
										self.error_mat_obs,
...
# 		assert not np.any(np.isnan(self.phi_x_tau_x_error)), ('phi_x_tau_x_error\n',self.phi_x_tau_x_error)
	...
	cdef _update_varpar_label_btm(self):
		cdef np.ndarray[np.float64_t, ndim=2] log_varpar_label = (
								self.mother.phi_x_tau_x_error_3d[self.id]
								+
								np.matmul(
...
# 		self._update_varpar_concent()
		self._update_varpar_stick()
# 		assert not np.any(np.isnan(self.varpar_stick)), ('stick\n',self.varpar_stick)
		self._update_varpar_label_btm()
# 		assert not np.any(np.isnan(self.varpar_label)), ('label\n',self.varpar_label)
		self._update_varpar_assignment_btm()
# 		assert not np.any(np.isnan(self.varpar_assignment)), ('assignment\n',self.varpar_assignment)
			
			
	def get_var_bound(self):
		return (
						self._get_sum_E_log_p_varpars()
						-self._get_E_log_q_varpar()
# 						+np.sum(
...
cdef class Word(object):
	...
	cdef readonly list ngrams
	def __init__(self, list string, int n, int id):
		self.id=id
		cdef tuple window
...
cdef class Ngram(object):
	cdef Word word
	cdef list restaurants, ids
	cdef readonly tuple context
	cdef readonly int target
	def __init__(self, tuple window, Word word):
		self.word=word
		self.restaurants = []
		self.ids = []
	...
	def enter_a_restaurant(self, HDPNgram hdp_ngram, int n):
# 		print self.context,hdp_ngram.tree[n-1]
		cdef DP_bottom restaurant = hdp_ngram.tree[n-1][self.context]
...
def code_data(data):
	str_data=[word.split(',') for word in data]
