...
# 		print np.exp(error_mat).sum(axis=1)
		...
		self.feature_ngrams = [HDPNgram(T_base, n, concent_priors, table_id, self.customers, self)
...
# 		np.savetxt(self.result_path+"sublexicalization_assignment.csv", self.varpar_assignment, delimiter=",")
		...
		for sblx_id,hdp in enumerate(self.feature_ngrams):
...
# 		self.feature_ngrams.update_varpars()
...
# 		self.intermediate_var_bound=new_KL
		[table.update_varpars() for table in self.feature_ngrams]
# 		new_KL=self.get_var_bound()
...
# 		print 'assign',new_KL-self.intermediate_var_bound
	...
	def get_var_bound(self):
		...
		"""
# 		return self.feature_ngrams.get_var_bound()
		...
		return (
				(
						self.concent_priors[0]*self.concent_log_mean
						-
				...
				+np.sum(
					[
						...
						for table in self.feature_ngrams
					]
					) 
				+(
