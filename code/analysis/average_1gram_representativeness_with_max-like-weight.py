...
def average_over_context(df_ngram, sublex_assignment_probs):
	sublex_assignment2others_probs = 1.0 - sublex_assignment_probs
	columns = ['sublex_id','value','expected_frequency','frequency','prob','prob_x_assignment','prob_x_assignment2others']
	new_df = pd.DataFrame(columns=columns)
	for sublex_id, df_per_context in df_ngram.groupby('sublex_id'):
	...
	for value, sub_df in new_df.groupby('value'):
	...
	return new_df.loc[:,['sublex_id','value','expected_frequency','frequency','prob','representativeness']]
...
if __name__ == '__main__':
	...
	df_ngram_shorter = average_over_context(df_ngram, assignment_probs)

	df_ngram_shorter.to_csv(os.path.join(result_dir, 'averaged-1gram-representativeness_max-like.csv'), encoding='utf-8', index=False)
