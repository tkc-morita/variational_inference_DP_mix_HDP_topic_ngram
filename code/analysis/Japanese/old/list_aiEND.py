...
if __name__ == '__main__':
	...
	df_ngram['context'] = df_ngram.decoded_context
	df_ngram['value'] = df_ngram.decoded_value

	context = ['ä_i']
	value = ['END']
	df_ngram = df_ngram[(df_ngram.context.isin(context)) & (df_ngram.value.isin(value))]
	...
	df_ngram.to_csv(os.path.join(result_dir, 'a-i-END_freq-at-least-%s.csv' % (str(min_frequency))), index=False, encoding='utf-8')
