...
def list_probable_words(
	...
	df.head(n=list_length).to_csv(os.path.join(result_dir, 'top-%i-probable_words_with-respect-to-sublex-%i.csv' % (list_length, sublex_id)), index=False, encoding='utf-8')
	df.tail(n=list_length).to_csv(os.path.join(result_dir, 'worst-%i-probable_words_with-respect-to-sublex-%i.csv' % (list_length, sublex_id)), index=False, encoding='utf-8')
...
if __name__ == '__main__':
	...
	if 'orthography' in df_data.columns:
		df['orthography'] = df_data.orthography
	else:
		df['orthography'] = df_data.lemma
	if 'DISC' in df_data.columns:
		df['DISC'] = df_data.DISC
	else:
		df['IPA_vsv'] = df_data.IPA_csv.str.replace(',','|')
