...
def list_probable_words(
	...
	df.head(n=list_length).to_csv(os.path.join(result_dir, 'top-%i-probable_monomorph-words_with-respect-to-sublex-%i.csv' % (list_length, sublex_id)), index=False, encoding='utf-8')
	df.tail(n=list_length).to_csv(os.path.join(result_dir, 'worst-%i-probable_monomorph-words_with-respect-to-sublex-%i.csv' % (list_length, sublex_id)), index=False, encoding='utf-8')
...
if __name__ == '__main__':
	...
	df_morph = pd.read_csv(sys.argv[3], sep='\t')

	# complexity = sys.argv[4]
	# df = df[~df_morph[complexity]]
	df = df[df_morph.MorphStatus.isin(['M'])]
	# print df
