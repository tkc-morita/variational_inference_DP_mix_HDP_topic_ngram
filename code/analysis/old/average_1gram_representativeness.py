...
def average_over_context(df_ngram):
	columns = ['sublex_id','value','representativeness','expected_frequency','frequency']
	new_df = pd.DataFrame(columns=columns)
	for (sublex_id, value), sub_df in df_ngram.groupby(['sublex_id', 'decoded_value']):
...
if __name__ == '__main__':
	...
	df_ngram_shorter = average_over_context(df_ngram)

	df_ngram_shorter.to_csv(os.path.join(result_dir, 'averaged-1gram-representativeness.csv'), encoding='utf-8', index=False)
